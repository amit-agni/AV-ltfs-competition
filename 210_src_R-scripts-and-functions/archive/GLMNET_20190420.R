...
train <- DT[flag =="train"]
train$flag <- NULL
train <- cbind(train,poly(train$ltv,20))

test <-DT[flag =="test",]
test$flag <- NULL
test$loan_default <- NULL
test <- cbind(test,poly(test$ltv,20))


cvmodel <- cv.glmnet(x=model.matrix(~.,train)[,-64],
                             y= model.matrix(~.,train)[,64]
                             ,family = "binomial"
                             ,alpha = 1
                     ,nfolds = 5)
plot(cvmodel)
...
predicted <- predict(cvmodel,
                     newx = model.matrix(~.,test)
                     , type = "response"
...
# glm.model <- glmn(factor(loan_default)~ .+poly(ltv,degree = 5)
#                   ,data = train
#                   ,family = binomial(link="logit"))

summary(glm.model)

predicted <- predict(glm.model,newx = model.matrix(~.,test), type = "response")
predicted
prob_pred <- ifelse(predicted > 0.5,1,0)
prop.table(table(prob_pred))
